...
PATH = "./advanced_model_best_params"
...
class MLP(nn.Module):
    ...
    def forward(self, lstm_out):
        ...
        for mod in range(sentence_length):
            ...
            x = self.non_linearity(self.dropout(summed_values))
...
class AdvancedDnnDependencyParser(nn.Module):
    def __init__(self, word_embedding_dim, pos_embedding_dim, hidden_dim, word_vocab_size, tag_vocab_size,
        ...
        if word_embedding_table is not None:
          lstm_input_size = 300 + pos_embedding_dim
        self.lstm = nn.LSTM(input_size=lstm_input_size, hidden_size=hidden_dim, num_layers=num_lst_layers,
                            bidirectional=True, batch_first=False,dropout=0.4)
...
def hyper_parameter_tuning():
    mlp_hidden_dim_arr = [200]
    EPOCHS = 150
    LSTM_LAYERS = [2, 4]
    word_embedding_dim_arr = [200]  # doesnt matter in pre-trained
...
def main():
    ...
    if use_validation:
      ...
      val_dataloader = DataLoader(val_set, shuffle=True)
    else:
    ...
    if USE_PRETRAINED:
        model = AdvancedDnnDependencyParser(WORD_EMBEDDING_DIM, POS_EMBEDDING_DIM, HIDDEN_DIM, word_vocab_size,
                                        tag_vocab_size, num_lst_layers = LSTM_LAYERS, word_embedding_table=glove.vectors).to(device)
    else:
        model = AdvancedDnnDependencyParser(WORD_EMBEDDING_DIM, POS_EMBEDDING_DIM, HIDDEN_DIM, word_vocab_size,
                                            tag_vocab_size, num_lst_layers = LSTM_LAYERS).to(device)
    if use_cuda:
        model.cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc < best_val_acc:     # no improvement
            ...
            plt.savefig('./advanced_model_graphs.png')
        ...
        print("epoch acc on test - ", test_acc)
